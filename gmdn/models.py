...
def insert(cnxn, input_file, table, fields, sheet_name):
    ...
    for index, row in df.iterrows():
        ...
        manufacturer = str(row['Manufacturer']).upper()
        try:
            gmdn = str(int(row['GMDN'])) # 'GMDN'
        except:
            gmdn = ''
        part_type = 1
        renewal_value = 1
        is_rfid = 0
        args = (model, manufacturer, gmdn, part_type, renewal_value, is_rfid)                         # *************** tuple of all arguments in correct order!
        key = (model, manufacturer)                                # *************** set key
        if key in key_check:
...
def load(cnxn, input_file, code):
    table_name = 'TYPES'                             # *************** insert table name as string
    fields = ['TP_TYPE', 'MARQUE', 'CNEH_TYPE', 'ASSET_PART_TYPE', 'REMP_PRIX', 'IS_RFID_TAG']                        # *************** insert the table field names
