...
def main():
    ...
    # empty manufacturers, gmdn and model tables
    cursor.execute("TRUNCATE TABLE MARQUES")
    cursor.execute("TRUNCATE TABLE EQ_CNEH")
    cursor.execute("TRUNCATE TABLE TYPES")
    conn.commit()

    # Manufacturers and GMDN are independent so can load either first
    ...
    data = pd.read_sql("SELECT * FROM EQ_CNEH", conn)
    print(data.head())
    sys.exit()
...
def insert_many(sql, records, connection, cursor):
    try:
        cursor.executemany("INSERT INTO EQ_CNEH (N_NOM_CNEH, NOM, NOM2) VALUES ")
