...
def insert(cnxn, input_file, table, fields, sheet_name):
    ...
    for index, row in df.iterrows():
        org_no = str(row['Organisation No'])
        pole_id = str(row['Pole ID'])
        local_code = chk_empty(str(row['Local Code']))
        name = chk_empty(str(row['Name']))
        args = (org_no, pole_id, local_code, name)                         # *************** tuple of all arguments in correct order!
        key = (pole_id)                                # *************** set key
        if key in key_check:
...
def load(cnxn, input_file, code):
    table_name = 'ACTIVITY_POLE'                             # *************** insert table name as string
    fields = ['FK_CUSTOMER_ID', 'ID', 'POLE_CODE', 'POLE_NAME']                        # *************** insert the table field names
    sheet_name = 'MED_ACT_POLE'                             # *************** insert the sheetname from excel
