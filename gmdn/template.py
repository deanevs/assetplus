...
def insert(cnxn, input_file, table, fields, sheet_name):
    ...
    for index, row in df.iterrows():
        arg1 = row['column header1']                # *************** add line for each value
        arg2 = row['column header2']
        args = (arg1, arg2)                         # *************** tuple of all arguments in correct order!
        key = (arg1)                                # *************** set key
        if key in key_check:
...
def load(cnxn, input_file, code):
    table_name = 'TABLENAME'                             # *************** insert table name as string
    fields = ['FIELD1', 'FIELD2']                        # *************** insert the table field names
    sheet_name = 'SHEETNAME'                             # *************** insert the sheetname from excel
