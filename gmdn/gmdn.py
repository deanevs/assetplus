...
def insert(cnxn, input_file, table, fields, sheet_name):
    ...
    for index, row in df.iterrows():
        gmdn_code = str(int(row['GMDN_CODE']))
        term_name = str(row['TERM_NAME']).upper()
        gmdn_def = str(row['GMDN_DEFINITION']).upper()
        status = str(row['STATUS']).upper()
        args = (gmdn_code, term_name, gmdn_def, status)                         # *************** tuple of all arguments in correct order!
        key = (gmdn_code)                                # *************** set key
        if key in key_check:
...
def load(cnxn, input_file, code):
    table_name = 'EQ_CNEH'                             # *************** insert table name as string
    fields = ['N_NOM_CNEH', 'NOM', 'NOM2', 'F_SPECL2']                        # *************** insert the table field names
    sheet_name = 'GMDN'                             # *************** insert the sheetname from excel
