...
def insert(cnxn, input_file, table, fields, sheet_name):
    ...
    for index, row in df.iterrows():
        acc_no = str(row['Account No']).upper()
        acc_name = str(row['Account Name']).upper()
        args = (acc_no, acc_name)                         # *************** tuple of all arguments in correct order!
        key = (acc_no)                                # *************** set key
        if key in key_check:
...
def load(cnxn, input_file, code):
    table_name = 'ACCOUNT'                             # *************** insert table name as string
    fields = ['NU_COMPTE', 'ACCOUNT_NAME']                        # *************** insert the table field names
    sheet_name = 'BUDGETARY_ACCOUNT'                             # *************** insert the sheetname from excel
