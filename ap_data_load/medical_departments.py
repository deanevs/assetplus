...
def insert(cnxn, input_file, table, fields, sheet_name):
    ...
    df_map = pd.read_excel(input_file, 'MED_ACT_POLE')
    ...
    for index, row in df.iterrows():
        No = str(row['No']).upper()
        name = str(row['Department Name']).upper()
        site = str(row['Site'])
        pole = chk_empty(str(row['Division Code']))
        cost = chk_empty(str(row['Cost Center No']))
        rc_num = chk_empty(str(row['RC No']))
        div_code = chk_empty(str(row['Division Code']))     # use this for lookup

        # ************** NON-STANDARD *****************
        fk_map = None   # init incase it is empty
        if div_code is not None:
            # we need to do a lookup of activity code
            for index, row in df_map.iterrows():
                if str(row['Local Code']) == div_code:
                    fk_map = int(row['Pole ID'])
                    break

        args = (No, name, site, fk_map, cost, rc_num)                         # *************** tuple of all arguments in correct order!
        key = (No)                                # *************** set key
        if key in key_check:
...
def load(cnxn, input_file, code):
    table_name = 'UNITES'                             # *************** insert table name as string
    fields = ['N_UF', 'N_SERVI', 'REF_PLAN', 'FK_ACTIVITY_POLE_ID', 'N_SERVI2', 'N_CENT_RES']                        # *************** insert the table field names
    sheet_name = 'MEDICAL_DEPARTMENT'                             # *************** insert the sheetname from excel
