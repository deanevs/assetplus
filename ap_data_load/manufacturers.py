...
def insert(cnxn, input_file, table, fields, sheet_name):
    ...
    run_tot = 0
    key_check = []
    for man in unique_manufacturers:
        manufacturer = str(man).upper()     # ensure string
        key = manufacturer
        if key in key_check:
...
def load(cnxn, input_file, code):
    table_name = 'MARQUES'                             # *************** insert table name as string
    fields = ['MA_NOM']                        # *************** insert the table field names
