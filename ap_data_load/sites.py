...
def insert(cnxn, input_file, table, fields, sheet_name):
    ...
    for index, row in df.iterrows():
        org_id = str(row['Organisation ID'])
        id = str(row['Site No']).upper()
        name = str(row['Name']).upper()
        city = str(row['City']).upper()
        postcode = str(row['Post Code']).upper()
        address = str(row['Address']).upper()
        args = (org_id, id, name, city, postcode, address)                         # *************** tuple of all arguments in correct order!
        key = (id)                                # *************** set key
        if key in key_check:
...
def load(cnxn, input_file, code):
    table_name = 'ETABLI'                             # *************** insert table name as string
    fields = ['FK_CUSTOMER_ID', 'N_ETAB', 'NOM_ETAB', 'VILLE_ETAB', 'CP_ETAB', 'ADR_ETAB']                        # *************** insert the table field names
    sheet_name = 'SITES'                             # *************** insert the sheetname from excel
